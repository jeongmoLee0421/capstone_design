...
void SendAll(SOCKET hSockArr[], char* msg, int strLen, int total); // 채팅 전송
		...
		for (i = startIdx; i < numOfClntSock; i++) {
			...
			if ((sigEventIdx == WSA_WAIT_FAILED || sigEventIdx == WSA_WAIT_TIMEOUT))
				...
				if (netEvents.lNetworkEvents & FD_READ) { // 데이터 수신
					...
					SendAll(hSockArr, msg, strLen, numOfClntSock); // 채팅 전송
				}
...
void SendAll(SOCKET hSockArr[], char* msg, int strLen, int total) {
